...
if(!defined('HAVE_PHP_SQL_PARSER')) {
	class PHPSQLParserOld {
		...
		#this function finds all the UNIONS and creates sub-queries from them? Not yet sure...
		function parse($sql) {
			$sql = trim($sql);
...
;
		...
		*/
		private function process_sql(&$tokens,$start_at = 0, $stop_at = false) {
			...
			for($token_number = $start_at;$token_number<$stop_at;++$token_number) {
				...
				switch($upper) {
					...
					case 'INDEX':
						if($token_category == 'CREATE') {
							...
							$token_category = $upper;
							continue 2;
						} else if ($token_category == 'FROM') {
						    $token_category = $modifier . ' ' . $upper;
						    continue 2;
						}
				...
				} else if (strpos($token_category, 'INDEX') !== false) {
			...
			if(!empty($out['IGNORE INDEX'])) $out['IGNORE INDEX'] = $this->process_expr_list($out['IGNORE INDEX']);
			if(!empty($out['USE INDEX'])) $out['USE INDEX'] = $this->process_expr_list($out['USE INDEX']);
		...
		*/
		function process_select_expr($expression) {
			$capture = false;
			$alias = "";
